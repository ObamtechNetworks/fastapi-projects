...
def test_vote_twice_on_post(authorized_client, test_posts, test_user, test_vote):
    response = authorized_client.post(f'/vote/', json={"post_id": test_posts[3].id, "dir": 1} )
    assert response.status_code == 409

def test_delete_vote(authorized_client, test_posts, test_user, test_vote):
    response = authorized_client.post(f'/vote/', json={"post_id": test_posts[3].id, "dir": 0} )
    assert response.status_code == 201
...
def test_vote_post_non_existent(authorized_client, test_posts):
    response = authorized_client.post(f'/vote/', json={"post_id": "80000", "dir": 1} )
