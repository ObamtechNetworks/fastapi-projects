...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
    ...
    new_post = models.Post(**post.model_dump(), owner_id=current_user.id) # unpack the post object into a dictionary and pass it to the Post model to create a new instance of the Post model
    db.add(new_post)
    db.commit()
...
@router.get("/latest", status_code=status.HTTP_200_OK, response_model=schemas.PostOut)
def get_latest_post(db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
    ...
    post = db.query(
        ...
        func.count(models.Vote.post_id).label("votes")
            ...
            models.Vote,
            models.Vote.post_id == models.Post.id,
            isouter=True).group_by(models.Post.id).order_by(models.Post.id.desc()).first()
...
def get_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
    post = db.query(
        ...
        func.count(models.Vote.post_id).label("votes")
            ...
            isouter=True).filter(
            models.Post.id == id
            ).group_by(models.Post.id).order_by(models.Post.id).first()
    if not post:
